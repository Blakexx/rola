...
void check_arch_table() {
  int dev = 0;
  ROLA_CUDA_CHECK(cudaGetDevice(&dev));
  ...
  cudaDeviceProp prop{};
  ROLA_CUDA_CHECK(cudaGetDeviceProperties(&prop, dev));
  const int cc = prop.major * 100 + prop.minor * 10;
  STD_TORCH_CHECK(denseref::arch::tabulated(cc),
  ...
  STD_TORCH_CHECK(denseref::arch::supported(want),
                  "UNSUPPORTED ARCHITECTURE at compute capability ", prop.major, ".", prop.minor,
  ...
  STD_TORCH_CHECK(want.smem_per_sm == smem_per_sm && want.smem_per_cta_max == smem_per_cta_max
                      && want.regs_per_sm == prop.regsPerMultiprocessor
                      && want.max_threads_per_sm == prop.maxThreadsPerMultiProcessor,
                  "the capability table disagrees with the driver at compute capability ",
                  prop.major, ".", prop.minor, ": table {smem/SM ", want.smem_per_sm, ", smem/CTA ",
