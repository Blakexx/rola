...
PassPlan build_plan(const std::vector<int64_t>& widths, int L, int64_t dense_read,
  ...
  TORCH_CHECK(D >= 1 && D <= kMaxLevels, "the routing depth is one to four levels, got ", D);
  TORCH_CHECK(L >= 1, "a call carries at least one token, got L=", L);
  ...
  for (int level = 0; level < D; ++level) {
    ...
    TORCH_CHECK(width >= 16 && width <= 256 && (width & (width - 1)) == 0, "level ", level, " is ",
                width,
  ...
  TORCH_CHECK(need <= kMaxMaskWords, "the digit mask needs ", need, " words; the block holds ",
              kMaxMaskWords);
  ...
  for (int side = 0; side < kSides; ++side) {
    TORCH_CHECK(modes[side] >= 0 && modes[side] < (1 << D), "side ", side,
                "'s dense-level bitmask ", modes[side], " names a level outside the depth ", D);
    TORCH_CHECK((int)masks[side]->size() == need, "side ", side, " carries ", masks[side]->size(),
                " digit-mask words; ", plan.rows, " rows need ", need);
...
const uint16_t* bits_of(const at::Tensor& plane) {
  return reinterpret_cast<const uint16_t*>(plane.const_data_ptr<at::BFloat16>());
}

}  // namespace

//: -- see docs/internals/facts/liveness_api.md#liveness-words
at::Tensor liveness_words(const at::Tensor& read_plane, const at::Tensor& write_plane,
  ...
  check_arch_table();
  for (const at::Tensor& plane : {read_plane, write_plane}) {
    TORCH_CHECK(plane.is_cuda(), "the pass reads device amplitude planes");
    TORCH_CHECK(plane.scalar_type() == at::kBFloat16, "the pass reads bf16 amplitudes, got ",
                plane.scalar_type());
    TORCH_CHECK(plane.is_contiguous(), "the amplitude plane is read as packed rows");
    TORCH_CHECK(plane.dim() >= 2, "an amplitude plane is [..., L, sum_l B_l], got ", plane.dim(),
                " dimensions");
  }
  TORCH_CHECK(read_plane.sizes() == write_plane.sizes(),
              "the two sides describe the SAME call: ", read_plane.sizes(), " vs ",
              write_plane.sizes());

  const int64_t rows = read_plane.size(-1);
  const int64_t L = read_plane.size(-2);
  ...
  TORCH_CHECK(plan.rows == rows, "the packed amplitude row is ", rows, " columns wide and the ",
              plan.D, " widths sum to ", plan.rows, ": a row index IS the column it votes from");

  at::Tensor out =
      at::empty({BH, (int64_t)kSides, rows, plan.words}, read_plane.options().dtype(at::kInt));
