...
| `ROLA_BUILD_PARTS` <a id="parts"></a> | **Iteration only.** `1` also builds `rola_cu13/_C_parts.abi3.so`, the part harness's driver (`measure/harness/bench_carry_parts.py`: the head, fill and fold components alone on a cell, KERNEL_STANDARDS §19), in the same pass beside the arm with `-lineinfo`. A torch JIT `load()` of the same three sources ran for minutes and was killed by the host's memory watchdog; a gating instrument that cannot run is a defect (§22). Never shipped. |
...
Every carry cell's call in the battery asks `tests/oracle/fixtures.py`'s `built_arms` whether the binary carries its arm first -- and that answer is the BINARY's, `rola.ops.carry.arms()`, never the source tree's. A declared test
...
its `manifests/` record, the ratification that record's `manifest_sha256` hashes. A future toolchain is a second plugin
...
2. **`tools/lint/lint_standards.py`** — the textual/preprocessor checks
   ...
   `docs/`, `.claude/skills/`, `measure/`, `tools/`, `CLAUDE.md`,
