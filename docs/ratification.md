...
`rola/_build_config.py` is generated by `setup.py` and answers *"which measured
evidence backs this exact `.so`?"* from an installed wheel with no repository
checkout:

```python
>>> from rola import _build_config; _build_config.show()
