...
the boxes deal evenly to the warps, a warp's state fits 128 registers a lane, the snapshot's
low-half stage aliases the readout/pool region at a call's edges (the two streams never alive
at once), the whole fits the architecture's maximum.
