...
(`benchmarks/unit/carry_parts/carry_parts.cu`), so a part runs on exactly the call the
...
(`make_side_layout`), and the parameter block filled but for the phase ledger
