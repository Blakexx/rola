...
def main() -> int:
    ...
    #: a fold.fragment event is a RUN's fragments (half a chunk's: kPoolTok / kTile / 2 of them at dense, two at
    #: this arm), eighteen HMMAs each; the first form divided by one fragment's and read the fold at 3.8x its pipe
    #: time, and before the second run was stamped its fragments were read as the mid-chunk fill's
    for lab, hmmas in (("fold.fragment", 2 * 18), ("readout.tile", 144)):
        if lab in durations:
            med = statistics.median(durations[lab])
