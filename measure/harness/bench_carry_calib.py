...
CALIBRATIONS = (
    ...
    ("lds_chain_1w", 4, 33, 16, 16, "dependent shared loads (each address the last result), one warp a scheduler: the latency"),
    ("lds_chain_2w", 8, 33, 16, 16, "the same, two warps a scheduler"),
    ("shfl_chain_1w", 4, 34, 16, 16, "dependent shuffles, one warp a scheduler: the latency"),
    ("ldsm_chain_1w", 4, 35, 16, 16, "dependent one-matrix ldmatrix loads, one warp a scheduler: the latency"),
    ("branch_taken_1w", 4, 36, 16, 16, "a taken uniform branch over a skipped block (setp + bra), one warp a scheduler"),
    ("branch_taken_2w", 8, 36, 16, 16, "the same, two warps a scheduler"),
    ("branch_divergent_1w", 4, 37, 16, 16, "odd and even lanes through different eight-add blocks and reconverge, one warp"),
    ("icache_256_2w", 8, 38, 256, 256, "a loop body of 256 independent multiply-adds (4 KB of code), two warps a scheduler"),
    ("icache_1024_2w", 8, 38, 1024, 1024, "1024 (16 KB)"),
    ("icache_2048_2w", 8, 38, 2048, 2048, "2048 (32 KB)"),
    ("icache_4096_2w", 8, 38, 4096, 4096, "4096 (64 KB)"),
    ("icache_8192_2w", 8, 38, 8192, 8192, "8192 (128 KB)"),
    ("icache_1024_1w", 4, 38, 1024, 1024, "1024 (16 KB), one warp a scheduler"),
